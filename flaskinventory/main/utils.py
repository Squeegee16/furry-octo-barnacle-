...
#######################################################################################
##### add in a check for location ######## nursery and green house don't have compost date
def growth_status(planted_date,dtm,germ):
    ...
    compostdate = datetime.datetime(todays_date.year,montoint(pdate[3:7]),int(pdate[0:2]))
    # print('BBBBB',todays_date.day,start_date.day)
    if todays_date.strftime("%j") == start_date.strftime("%j"):
        days =0
    else:
...
def db_convert(data):
    ...
    try:
        ...
        print('db to str converstion error')
...
#https://sunrise-sunset.org/api
    ...
    if er == 1:
        ...
        sun_set =  str(str2int2str(ss[1],tz)+':'+ss[2]+':'+ss[3].strip('AM,"')+'PM')
        day_len = str(dl[1].strip('"')+':'+dl[2]+':'+dl[3].strip('"'))
        dwnst = str(str2int2str(dwnst[1],tz)+':'+dwnst[2]+':'+dwnst[3].strip('"'))
        dwnsp = str(str2int2str(dwnsp[1],tz)+':'+dwnsp[2]+':'+dwnsp[3].strip('AM,"')+'PM')
...
def stripper(data):
    ...
    for i in range(len(u)):
        ...
        h.append(stripper(db_convert( /
        Product.query.filter_by(variety=u[i][4]).first())))
...
#######################################################################################
def pdf_template(Rpt_type,data ):
    rpt = Rpt_type
    data = stripper(db_convert(data))
    pdf = FPDF()
#header of the pdf file
    header = 'Plantrix Garden Management Report'
    pdf.add_page()
    ...
    if rpt == 0:
        ...
        pdf.cell(0, 6, 'Plant and Seed %d : %s' % (1, 'Library'), 0, 1, 'L', 1)
        #########
        page_width = pdf.w - 2 * pdf.l_margin
        col_width = page_width/4 
        th = pdf.font_size
        for row in range(len(data)):
            pdf.cell(col_width, th, data[row], border=1)
        ...
        pdf.multi_cell(0, 5, 'Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s,')

    elif rpt == 2:
        pdf.ln(10)
        pdf.set_font('Times', '', 12)
        pdf.multi_cell(0, 5, 'Lorem Ipsum is simply dummy text ')

    elif rpt == 3:
        pdf.ln(10)
        pdf.set_font('Times', '', 12)
        pdf.multi_cell(0, 5, 'Lorem Ipsum.')

    else:
        flash(f' Report Error ','danger')
