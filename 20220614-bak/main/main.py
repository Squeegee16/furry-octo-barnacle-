        ...
        if frm == to :
            ...
            if prodq.prod_qty >= qty:
                ...
                if(a=='None'):
                    # Aused = cal_space(locSpace.loc_area,prod.SspaceMin,qty)
                    # print('new out',Aused)
                    new = Balance(product=name,var=var,location=to,quantity=qty,usedSpace=0.0,availSpace=0.0)
                    db.session.add(new)
                    db.session.commit()
                else:
                    bal.quantity += qty
                    # Aused = cal_space(locSpace.loc_area,prod.SspaceMin,bal.quantity)
                    # print('old out',Aused)
                    bal = Balance.query.filter_by(location=to,product=name).update(dict(quantity = bal.quantity,usedSpace=0.0,availSpace=0.0))
                    db.session.commit()
            else:
                return False
...
def locstatCard():
    ...
    if request.method == 'GET':
        if dat == None:
            ...
            print('AAAAA',dat.planted_date,plt.DTMat,plt.germination)    
